...
- El jugador aparece en una casilla caminable cercana al centro del laberinto.
- Los pacgums no se generan sobre celdas bloqueadas (`F` en la matriz del laberinto).
- La salida visible del mapa fue eliminada del render.
- Al vaciar todos los coleccionables, el juego avanza automáticamente al siguiente nivel si existe uno.
...
### Política de generación (requisito de subject)
...
- Se detectó que algunos seeds tardan demasiado y disparan timeout.
...
### Spawn central y progreso de nivel
- El spawn se calcula buscando la casilla accesible más cercana al centro del mapa.
- Esa posición ya queda preparada como base para respawn futuro.
- Al limpiar todos los coleccionables, si quedan niveles, se carga el siguiente automáticamente.

## 3) Archivos clave tocados
- `src/pacman/game.py`
- `src/pacman/maze_provider.py`
- `config.json`
- `informes/informe_estado_2026-04-26.md`
...
- Pruebas de comportamiento manual/controlado:
  ...
  - `Q` global en `menu`, `loading`, `playing`, `game_over` y `error`.
    - Spawn central verificado con un laberinto sintético.
    - Pacgums no colocados en celdas bloqueadas.
    - Progreso automático al siguiente nivel verificado con un test sintético.
...
- Con la política “sin fallback”, un timeout debe mostrarse como error.
- La calidad de “aleatoriedad” está condicionada por el pool de seeds seguros para evitar bloqueos/timeout.
- El juego todavía no implementa la parte final del subject: vidas, fantasmas, name entry, highscores en menú, pausa y temporizador visible.
...
3. Entrada de nombre al finalizar partida.
4. Persistencia y visualización de highscores en menú.
5. Uso de `level_max_time` y temporizador en HUD.
6. Pausa y reanudación durante la partida.
...
2. Si aparecen más timeouts, priorizar:
   - ampliar o recalibrar la lista de seeds rápidas según el tamaño del mapa,
