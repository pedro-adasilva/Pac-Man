...
1. Lógica real de pacgums (spawn, render, recogida, puntuación).
2. Sistema de vidas y condiciones de derrota.
3. Fantasmas (spawn, movimiento/IA básica, colisiones).
4. Progresión multi-nivel (usar toda la lista `levels`).
5. Entrada de nombre al finalizar partida.
6. Persistencia y visualización de highscores en menú.
7. Uso de `level_max_time` y temporizador en HUD.
...
2. Si aparecen más timeouts, priorizar:
   - optimizar selección de seeds válidos,
   - o introducir reintentos limitados con otros seeds del pool,
